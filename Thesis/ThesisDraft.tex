...
Most basic case for parabolic differential equations is the heat equation. In the experiments the following initial and boundary conditions will be used. We will consider the following PDE and conditions for the purposes of research.
...
We will be applying the finite differences to the equation  \ref{gen}  for the purposes of simplicity since heat equation and Black-Scholes equation can be generalized in the form for certain choices of coefficients. 

\begin{equation}
a(t, x)  \frac{u^n_{i+1}- 2u^n_i + u^n_{i-1}}{(\Delta x)^2}  + b(t, x)  \frac{u^{n}_{i+1} - u^{n}_{i-1}}{\Delta x} + c(t, x)   \frac{u^{n+1}_i - u^n_i}{\Delta t} + d(t, x) u^n_i = 0
\end{equation}

Applying the forward time and centred space differences where $ r = \frac{\Delta t}{\Delta x^2} $.
\begin{multline}
u^{n+1}_i = u^n_{i + 1} (\frac{-r a(t,x)}{\Delta x c(t,x)} - \frac{a(t, x)}{c(t, x)}) + u^n_{i}(1 + \frac{2 r a(t, x)}{c(t, x)} - \frac{\Delta t d(t, x)}{c(t, x)}) \\
+ u^n_{i - 1} (\frac{-r a(t, x)}{c(t, x)} + \frac{r b(t, x)}{\Delta x c(t, x)})
\end{multline}
...
The formula expresses one unknown nodal value directly in terms of known nodal values  \cite{evans}.  In the case of heat equation the coefficients  $\gamma,  \alpha, \beta$ become
\begin{equation}
\alpha =  r \hspace{15pt} \beta = 1 - 2r \hspace{15pt} \gamma = r
\end{equation}

Replacing the coefficients with black scholes partial differential equation coefficients, the coefficients $\gamma,  \alpha, \beta$ yields
\begin{equation}
...
Solving heat equation and Black-Scholes differs only in one aspect. Black-Scholes is solved backwards in time where heat equation is solved forwards in time. 
...
The explicit method is computationally cheap however this brings a serious drawback, for explicit method to attain reasonable accuracy the step size must be kept small \cite{gdsmith}. Thankfully, the Crank-Nicolson finite difference scheme was introduced by John Crank and Phyllis Nicolson \cite{cn}. Considering numerous articles and publications in the financial engineering literature use Crank-Nicolson as the de-facto scheme for time discretisation, the method has become one of the most popular finite difference schemes for approximating the solution of the Black - Scholes equation and its generalisations \cite{tavella}.
 ...
 If we apply backwards time difference instead of forward time difference that Explicit method used and a central space approximation in space again, we get the BTCS scheme. Applying the finite differences to the equation \ref{gen} 
\begin{equation}

...
\begin{definition} The resulting partial differential equation can be solved for the analytical solution.  The boundary conditions and initial conditions for the call option is
\begin{eqnarray}
...
First, we need to discretize the PDE for numerically solving parabolic PDEs such as heat equation using FDMs, the space dimension(s) and the time dimension are handled differently.

\begin{figure}[!htb]
    \centering
        \includegraphics[scale=0.8]{Discretize.png}
    \caption{10 x 10 discretization.}
\end{figure}
    
\begin{figure}[!htb]
    \centering
        \includegraphics[scale=0.8]{Boundary.png}
    \caption{Boundary conditions.}
\end{figure}

\begin{figure}[!htb]
    \centering
        \includegraphics[scale=0.8]{Initial.png}
...
Main optimization techniques that can be implemented can be listed as follows
...
In practical terms, you would use X64 when:  You need to directly address more than 4GB of memory, or
    You need very fast (native) processing of 64 bit numerical quantities (including double-precision floating-point numbers) X86 (32 bit) is suitable for most everything else.
